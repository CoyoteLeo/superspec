...
When you start building something, Superspec doesn't jump into code. It walks you through a design conversation, writes a spec, creates an implementation plan with persistent task tracking, and then executes it with subagent-driven development and a per-task code review that gates on both spec compliance and code quality.
...
ss-writing-plans → plan.md + tasks.md
       ↓
execute (ss-subagent-driven-development) → worktree + stacked PRs → tasks.md checkbox tracking
       ↓
...
3. **ss-subagent-driven-development** — Fresh subagent per task, each committing its own work, followed by one task review that returns both a spec-compliance and a code-quality verdict over the committed diff. Tracks progress in both TodoWrite (in-session) and `tasks.md` (persistent — commit ranges, review verdicts, and open PRs, so it survives compaction). Works in a git worktree and ships each PR-sized group as its own stacked PR, then verifies CI and answers every review comment inline. Keeps the stack current by rebasing its own branches (`--force-with-lease` only, never a base or shared branch), and cleans up the worktree and branch once you confirm a merge. It never merges — that stays with you.
...
| **ss-subagent-driven-development** | Per-task subagent execution with a spec + quality review over each task's committed diff |
